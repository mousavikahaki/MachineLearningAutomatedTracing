...
x = Conv3D(32, (3, 3, 3), padding='same', activation='relu')(input1)# change to leakyRelu to avoid dead neurons
...
out1 = keras.layers.Dense(1, activation='sigmoid')(out)# try Logistic, tanh,softmax and their combination(how to do combination?)
...
history = model.fit([IMsTrain3D.reshape(IMsTrain3D.shape[0], IMsTrain3D.shape[1], IMsTrain3D.shape[2], IMsTrain3D.shape[3],1),XFeature_train,XScenarios_train],
                    yIMs_train,
                    epochs=epoch,
...
#######                Save Model
# model.save('E:/AutomatedTracing/Data/Models/ScenarioConnectome/CONVOLUTION_IM_'+str(ImagetoTest)+'_out.h5')
...
#######                Predict Values
y_pred = model.predict([IMsTest.reshape(IMsTest.shape[0], IMsTest.shape[1], IMsTest.shape[2], IMsTest.shape[3],1),XFeature_test,XScenarios_test])
y_pred = y_pred[:,0]
y_pred.shape
...
# sio.savemat('E:/AutomatedTracing/Data/TrainingData/scenarios_images_features/Matrix_Predict_IM_'+str(ImagetoTest)+'_out.mat',{"y_pred":y_pred_Final})
